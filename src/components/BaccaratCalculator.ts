export class BaccaratCalculator extends HTMLElement {
  netUnits: number;
  steps: number;
  unitHistory: number[];
  ...
  connectedCallback() {
    this.storageKey = this.getAttribute("storage-key") || "baccarat_sessions";
    try {
  ...
  setupEventListeners() {
    this.shadowRoot!.getElementById("btn-calc-minus")!.addEventListener(
      "click",
      () => this.handleUnitChange(-1),
    );
    this.shadowRoot!.getElementById("btn-calc-plus")!.addEventListener(
      "click",
      () => this.handleUnitChange(1),
    );
    this.shadowRoot!.getElementById("btn-calc-undo")!.addEventListener(
      "click",
      () => this.handleUndoUnitChange(),
    );
    this.shadowRoot!.getElementById("btn-calc-refresh")!.addEventListener(
      "click",
      () => this.handleRefreshCalculator(),
    );
    this.shadowRoot!.getElementById("btn-calc-log")!.addEventListener(
      "click",
      () => this.toggleCalcLog(),
    );
    ...
    this.shadowRoot!.getElementById("btn-calc-clear-all")!.addEventListener(
      "click",
      () => this.clearAllSessions(),
    );
    this.shadowRoot!.getElementById("btn-modal-cancel")!.addEventListener(
      "click",
      () => this.closeModal(),
    );
    this.shadowRoot!.getElementById("btn-modal-confirm")!.addEventListener(
      "click",
  ...
  updateUI() {
    ...
    if (this.netUnits > 0) {
      calcNetValue.classList.add("positive");
    } else if (this.netUnits < 0) {
    ...
    ).disabled = this.unitHistory.length === 0;
    ...
    const btnCalcClearAll =
      this.shadowRoot!.getElementById("btn-calc-clear-all")!;
    ...
    if (this.sessions.length === 0) {
      ...
      if (calcSubtotalPanel) {
        ...
        calcSubtotalPanel.className = "calc-subtotal-panel";
        if (subtotal > 0) {
          calcSubtotalValue.classList.add("positive");
          calcSubtotalPanel.classList.add("positive");
        } else if (subtotal < 0) {
          calcSubtotalValue.classList.add("negative");
          calcSubtotalPanel.classList.add("negative");
        }
      }
      ...
      // Group by date
      const groupedSessions = this.sessions.reduce(
        (acc, session) => {
      ...
      Object.entries(groupedSessions).forEach(([date, dateSessions]) => {
        ...
        (dateSessions as any[]).forEach((session) => {
          const tr = document.createElement("tr");
          const timePart = session.date.split(",")[1]
            ? session.date.split(",")[1].trim()
            : session.date;
          const netClass =
            session.net > 0
    ...
    this.shadowRoot.innerHTML = `
            <style>
                ...
                * {
                ...
                .calc-subtotal-panel.positive {
                    background-color: rgba(6, 78, 59, 0.3);
                    border-color: rgba(6, 78, 59, 0.4);
                }

                .calc-subtotal-panel.negative {
                    background-color: rgba(159, 18, 57, 0.3);
                    border-color: rgba(159, 18, 57, 0.4);
                }
                ...
                .calc-subtotal-value.positive {
                    color: var(--emerald-400);
                }

                .calc-subtotal-value.negative {
                    color: var(--rose-400);
                }
            </style>
            ...
            <div class="calc-container">
                ...
                <div id="calc-log-panel" class="calc-log-panel hidden">
                    <div id="calc-subtotal-panel" class="calc-subtotal-panel hidden">
                        <span class="calc-subtotal-label">Subtotal</span>
